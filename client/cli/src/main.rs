...
use qubes_app_linux_converter_client_core::{convert_all_files, ConvertParameters, ConvertEvent};
use std::thread;
...
fn main(){
    ...
    files.dedup();
    let parameters = ConvertParameters{in_place: opts.in_place, archive: opts.archive, files: files.clone()};
    ...
    for convert_status in receiver_convert_events{
        match convert_status{
            ConvertEvent::FileInfo{output_type, number_pages, file} => println!("{}", number_pages),
            ConvertEvent::PageConverted{file, page} => println!("{}", file),
            ConvertEvent::FileConverted{file} => {files.retain(|x| *x != file); println!("{}", file);},
